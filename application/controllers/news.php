...
class News extends CI_Controller {
	...
	public function Information(){
		...
		$page =array(
			'total'=>ceil(sizeof($textlist)/5),
			'previous'=>$p-1 <= 0 ? 1 : $p-1,
			'next'=>$p+1 > ceil(sizeof($textlist)/5) ? ceil(sizeof($textlist)/5) : $p+1,
			'last'=>ceil(sizeof($textlist)/5),
		);
		if($p == 1){
			$textlist=array_slice($textlist,0,5);
		}else{
			$start=($p-1)*5+1;
			$textlist=array_slice($textlist,$start,5);
		}		
